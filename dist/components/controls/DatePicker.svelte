<script lang="ts" module>import { getLocale } from "@juvofy/lib/App.svelte";
import Calendar, {} from "./Calendar.svelte";
const placeholderDate = new Date(2e3, 9, 15);
</script>

<script lang="ts" generics="Type extends CalendarType">const componentId = $props.id();
const locale = getLocale();
const placeholder = placeholderDate.toLocaleDateString(locale).replace(/\d/g, "x");
let { value = $bindable(), id = componentId, class: customClass, ...rest } = $props();
</script>
