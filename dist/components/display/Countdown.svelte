...
<script lang="ts">const { value, class: customClass, variant, ...props } = $props();
...
function getDifference() {
  const difference2 = end - Date.now();
  const days = Math.floor(difference2 / 864e5);
  const hours = Math.floor(difference2 % 864e5 / 36e5);
  const minutes = Math.floor(difference2 % 36e5 / 6e4);
  const seconds = Math.floor(difference2 % 6e4 / 1e3);
...
$effect(() => {
  timeoutId = window.setTimeout(function set() {
    ...
    timeoutId = window.setTimeout(set, 1e3);
  }, 1e3);
